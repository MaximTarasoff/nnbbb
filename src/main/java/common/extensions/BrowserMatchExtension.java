...
public class BrowserMatchExtension implements ExecutionCondition {
    ...
    public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext extensionContext) {
        // логика по которой тест проходиться или пропускается
        ...
        boolean matches = Arrays.stream(annotation.value()).anyMatch(browser -> browser.equals(currentBrowser));

        if (matches) {
            return ConditionEvaluationResult.enabled("Текущий браузер удовлетворяет условию: " + currentBrowser);
        } else {
            return ConditionEvaluationResult.disabled("Тест пропущен так как текущий браузер: " + currentBrowser + " не находится в списке допустимых для теста: " + Arrays.toString(annotation.value()));
        }
    }
}
