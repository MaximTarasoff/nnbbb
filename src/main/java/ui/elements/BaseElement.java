...
public abstract class BaseElement {
    ...
    protected SelenideElement find(By selector) {
        return element.find(selector); //искать разные параметры внутри элемента
    }

    protected SelenideElement find(String cssSelector) {
        return element.find(cssSelector); //искать разные параметры внутри элемента
    }

    protected ElementsCollection findAll(By selector) {
        return element.findAll(selector); //искать разные параметры внутри элемента
    }
